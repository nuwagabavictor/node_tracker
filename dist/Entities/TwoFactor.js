"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var TwoFactor_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.TwoFactor = void 0;
const typeorm_1 = require("typeorm");
const User_1 = require("./User");
let TwoFactor = TwoFactor_1 = class TwoFactor extends typeorm_1.BaseEntity {
    id;
    otpHash;
    user;
    userId;
    expiresAt;
    used;
    createdAt;
    updatedAt;
    markUsed() {
        this.used = true;
    }
    isExpired() {
        return new Date() > this.expiresAt;
    }
    isValid() {
        return !this.used && !this.isExpired();
    }
    static createForUser(params) {
        const factor = new TwoFactor_1();
        factor.user = params.user;
        factor.otpHash = params.otpHash;
        factor.expiresAt = new Date(Date.now() + params.expiryMinutes * 60 * 1000);
        factor.used = false;
        return factor;
    }
};
exports.TwoFactor = TwoFactor;
__decorate([
    (0, typeorm_1.PrimaryGeneratedColumn)(),
    __metadata("design:type", Number)
], TwoFactor.prototype, "id", void 0);
__decorate([
    (0, typeorm_1.Column)({ length: 255, nullable: false }),
    __metadata("design:type", String)
], TwoFactor.prototype, "otpHash", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => User_1.User, { nullable: false, onDelete: "CASCADE" }),
    (0, typeorm_1.JoinColumn)({ name: "user_id" }),
    __metadata("design:type", User_1.User)
], TwoFactor.prototype, "user", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'user_id' }),
    (0, typeorm_1.Index)(),
    __metadata("design:type", Number)
], TwoFactor.prototype, "userId", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: "datetime" }),
    __metadata("design:type", Date)
], TwoFactor.prototype, "expiresAt", void 0);
__decorate([
    (0, typeorm_1.Column)({ default: false }),
    __metadata("design:type", Boolean)
], TwoFactor.prototype, "used", void 0);
__decorate([
    (0, typeorm_1.CreateDateColumn)(),
    __metadata("design:type", Date)
], TwoFactor.prototype, "createdAt", void 0);
__decorate([
    (0, typeorm_1.UpdateDateColumn)(),
    __metadata("design:type", Date)
], TwoFactor.prototype, "updatedAt", void 0);
exports.TwoFactor = TwoFactor = TwoFactor_1 = __decorate([
    (0, typeorm_1.Entity)("two_factor")
], TwoFactor);
//# sourceMappingURL=TwoFactor.js.map